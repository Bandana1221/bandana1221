...
import com.laviton.dao.PropertyDao;
...
import JsonInput.ProprtyRequest;
import JsonOutput.PropertyResponse;
...
*/
	 ...
	 @POST
	    @Path("/databydateweekly")
	    @Produces(MediaType.APPLICATION_JSON)
	 @Consumes(MediaType.APPLICATION_JSON)
	 public Response sendTest(InputJsonDates input) throws JSONException, JsonParseException, JsonProcessingException, IOException, ParseException {
		OutputJsons oj=new OutputJsons();
		MonthlJson mj=new MonthlJson();
		ArrayList<OutPutJson> datas=new ArrayList<OutPutJson>();
		MeterDetailsService sv=new MeterDetailsService();

		if(input.getAccestoken().equals(LavitonConstants.AccessToken))
		{
		if(input.getEdate()!=null)
		{
		...
		if(input.getSdate()!=null)
		{
			if(input.getReqtype().toUpperCase().equals("WEEKLY"))
				
			{
				oj=sv.startdatedataweekly(input.getSdate());
			}
	 ...
	 @POST
	    @Path("/databydatemonthly")
	    @Produces(MediaType.APPLICATION_JSON)
	 @Consumes(MediaType.APPLICATION_JSON)
	 public Response sendTestmonthly(InputJsonDates input) throws JSONException, JsonParseException, JsonProcessingException, IOException, ParseException {
		OutputJsons oj=new OutputJsons();
		MonthlJson mj=new MonthlJson();
		MeterDetailsService sv=new MeterDetailsService();
		...
		if(input.getEdate()!=null)
		{
		 ...
		 if (input.getReqtype().toUpperCase().equals("MONTHLY")) {
			 mj=sv.monthdatas(input.getEdate());
		...
		if(input.getSdate()!=null)
		{
			 ...
			 if (input.getReqtype().toUpperCase().equals("MONTHLY")) {
				 mj=sv.monthstartdatas(input.getSdate());
			 ...
			 mj.setError("Invalid Request Type");
		 }
		 return Response
		            .status(200)
		            .header("Access-Control-Allow-Origin", "*")
	 ...
	 @POST
	    ...
	    @Produces(MediaType.APPLICATION_JSON)
	 @Consumes(MediaType.APPLICATION_JSON)
	 public Response addProrty(ProprtyRequest input) throws JSONException, JsonParseException, JsonProcessingException, IOException, ParseException {
		PropertyResponse response=new PropertyResponse();
		PropertyDao dao=new PropertyDao();
		com.laviton.model.Property prop=new com.laviton.model.Property();
		
		if(input.getAccesstoken().equals(LavitonConstants.AccessToken))
		{
			if(input.getPropertyaddress()!=null|| input.getPropertyname()!=null)
			{
				prop.setProperty_address(input.getPropertyaddress());
				prop.setProperty_name(input.getPropertyname());
				dao.addmeterLogs(prop);
			}
			else
			{
				response.setError("Please Enter Address and Name Properly");
				response.setSuccess("false");	
			}
		
		
		int id=dao.getlogs(prop);
		if(id!=0)
	...
	if(tennant!=null)
	{
			...
			dao.removeallmeter(Integer.parseInt(input.getTennantid()));
