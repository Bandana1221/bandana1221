...
public class MeterDetailsService {
	...
	private static Logger LOG = Logger.getLogger(MeterDetailsService.class
			.getName());

	public void addLogs() {
		System.out.println(MeterDetailsService.class.getClassLoader()
				.getResource("/home/suraj/logfiles/log4j.properties"));

	}

	public void addMeterDetails(String csvFile) throws ParseException,
					...
					while ((line = br.readLine()) != null) {
						...
						if (i != 0) {
							...
							SimpleDateFormat sdf = new SimpleDateFormat(
									"yyyy-MM-dd HH:mm:ss");
							java.util.Date dat = (java.util.Date) sdf
									.parse(date);
							meterLogs.setTime(dat);
							LocalDate now = dat.toInstant()
							...
							if (!country[1].equals("0")) {
								...
								if (lineno == 1) {
									...
									// prints
									LOG.info("An error File is deleted"
											+ f3.getPath());
								...
								meterLogs
										.setMeter_log_total_positive_energy(Float
												.parseFloat(country[4]));
								meterLogs
										.setMeter_log_total_negative_energy(Float
												.parseFloat(country[5]));
								meterLogs.setMeter_log_peak_demand(country[6]);
								add.add(meterLogs);
								dao.addmeterLogs(meterLogs);
								pathinserted.add(path);
								LOG.info("Entry Successfully added with time"
										+ country[0] + "Energy Reading"
										+ country[4]);
	...
	public OutputJsons weekdatas(String date) throws ParseException,
			JSONException, SocketException, IOException {
		float total=0.0f;
	...
	public OutputJsons startdatedataweekly(String date) throws ParseException {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		java.util.Date dat = (java.util.Date) sdf.parse(date);
		ArrayList<String> dates = xml.listOfstartdates(dat, 7);
		ArrayList<OutPutJson> oj = new ArrayList<OutPutJson>();
		JSONArray ja = new JSONArray();
		float penergy[] = new float[100];
		int i = 0;
	...
	public OutputJsons rangeweekdatas(String date, String date2)
			throws ParseException, JSONException, SocketException, IOException {
		XmlExtract xml = new XmlExtract();
		OutputJsons out = new OutputJsons();
		MeterLogsDao dao = new MeterLogsDao();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
	...
	public MonthlJson monthdatas(String date) throws ParseException,
			JSONException, SocketException, IOException {
		XmlExtract xml = new XmlExtract();
		float total=0.0f;
		MonthlJson out = new MonthlJson();
		ArrayList<Weekdatas> oj = new ArrayList<Weekdatas>();
		ArrayList<MeterLogs> meter = new ArrayList<MeterLogs>();
		MeterLogsDao dao = new MeterLogsDao();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		java.util.Date dat = (java.util.Date) sdf.parse(date);
		LocalDate now = dat.toInstant().atZone(ZoneId.systemDefault())
				.toLocalDate();
		...
		int x = -1;
		while (days > 0) {
			ArrayList<String> dates = xml.Dates(dat, 7);
			JSONArray ja = new JSONArray();
			float penergy[] = new float[100];
			int i = 0;

			ArrayList<MeterLogs> mt1 = dao.getlogs(dates.get(i));
			ArrayList<MeterLogs> mt2 = dao.getlogs(dates.get(1));
			if (mt2 != null) {
				dates = xml.Dates(dat, 6);
				mt1 = dao.getlogs(dates.get(i));
				mt2 = dao.getlogs(dates.get(1));
			...
			if (mt1 != null)
				penergy[i] = xml.getenergyConsumedMaximum(mt1);
			else
				penergy[i] = 0.0f;
			if (mt2 != null)
				penergy[i + 1] = xml.getenergyConsumedMinimum(mt2);
			else
				penergy[i + 1] = 0.0f;
			Weekdatas o = new Weekdatas();
			
				if (penergy[i + 1] == 0.0f) {
					o.setDate("Week" + (x++));
					o.setPenergy("No Weekly data availabe");
				} else {
					o.setDate("Week" + (x++));
					o.setPenergy("" + (penergy[i] - penergy[i+1]) + "kwh");
					total=total+(penergy[i] - penergy[i+1]);
				}
			...
			LocalDate now1 = dat.toInstant().atZone(ZoneId.systemDefault())
					.toLocalDate();
			LocalDate then = now.minusDays(7);
			dat=Date.from(then.atStartOfDay(ZoneId.systemDefault()).toInstant());
		}
	...
	public MonthlJson monthstartdatas(String date) throws ParseException,
			JSONException, SocketException, IOException {
		XmlExtract xml = new XmlExtract();
		float total=0.0f;
		MonthlJson out = new MonthlJson();
		...
		int x = -1;
		while (days > 0) {
			ArrayList<String> dates = xml.startdates(dat, 7);
			JSONArray ja = new JSONArray();
			float penergy[] = new float[100];
			int i = 0;
			...
			Weekdatas o = new Weekdatas();
				...
				if (penergy[i + 1] == 0.0f || penergy[i] == 0.0f) {
					o.setDate("Week" + (x++));
					o.setPenergy("No Weekly data availabe");
				} else {
					o.setDate("Week" + (x++));
					o.setPenergy("" + (penergy[i+1] - penergy[i]) + "kwh");
					total=total+(penergy[i+1] - penergy[i]);
				}
