...
public boolean deletetennant(TennantDetails tennantdetails)
{
	boolean value=false;
	 sessionFactory = new Configuration().configure().buildSessionFactory();
		Session session=sessionFactory.openSession();
...
public void removeallmeter(int tennantid)
{ArrayList<TennantDetails> meter1=new ArrayList<TennantDetails>();
	 ...
	 sessionFactory = new Configuration().configure().buildSessionFactory();
		...
		try{
			 ...
			 Criteria criteria1 = session.createCriteria(TennantDetails.class).add(Restrictions.eq("tennant_id",tennantid));
				...
				meter1= (ArrayList<TennantDetails>) criteria1.list();
				System.out.println(meter1.size());
				Criteria criteria2 = session.createCriteria(MeterDetails.class);
				tennant=meter1.get(0);
				Set<MeterDetails> mt=tennant.getMeetings();
				if(mt!=null){
				for(MeterDetails m:mt)
				{
					tennant.getMeetings().remove(m);
					m.setMeterassign("false");
				}
			//tennant.getMeetings().clear();
			session.saveOrUpdate(tennant);
			session.delete(tennant);
			System.out.println("datas deleted");
				}
